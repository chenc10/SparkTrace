...
class Stage:
  ...
  def __str__(self):
   ...
   s = (("%s tasks (avg runtime: %s) StartTime: %s-%s-%s, FinishTime: %s-%s-%s, RunTime: %s-%s, ") %
           (len(self.tasks), self.average_task_runtime(), str(self.start_time - time_0)[:7], str(self.start_time - time_0)[7:10], str(self.start_time - time_0)[10:],
            str(self.finish_time() - time_0)[:7], str(self.finish_time() - time_0)[7:10], str(self.finish_time() - time_0)[10:],
	    str(self.finish_time() - self.start_time)[:-3], str(self.finish_time() - self.start_time)[-3:]))
   for i in range(len(self.tasks)):
     s = s + "\n      task-%d: start: %s-%s-%s, finish: %s-%s-%s, run: %s-%s" % \
             (i, str(self.tasks[i].start_time - time_0)[:7], str(self.tasks[i].start_time - time_0)[7:10], str(self.tasks[i].start_time - time_0)[10:],
              str(self.tasks[i].finish_time - time_0)[:7], str(self.tasks[i].finish_time - time_0)[7:10], str(self.tasks[i].finish_time - time_0)[10:],
	      str(self.tasks[i].runtime())[:-3], str(self.tasks[i].runtime())[-3:])

   return s
#    return (("%s tasks (avg runtime: %s) Start: %s-%s, Finish: %s-%s, runtime: %s, ") %
#            (len(self.tasks), self.average_task_runtime(), str(self.start_time)[:9], str(self.start_time)[9:],
#             str(self.finish_time())[:9], str(self.finish_time())[9:],
